...
interface Props {
  ...
  setCheckedTasks: React.Dispatch<React.SetStateAction<{ [key: string]: boolean }>>;
  isChecked:boolean;
...
const OverlayStatusbar: React.FC<Props> = ({ checkedTasks = {}, setCheckedTasks, setIsChecked, taskList, setTaskList, filteredTasks, setFilteredTasks }) => {
  ...
  const handleDeleteTask = () => {
    ...
    const newTaskList = taskList.filter((task) => !checkedTaskIds.includes(task.id))
    setTaskList(newTaskList)
    console.log("newTaskList", newTaskList);

    const newFilteredTaskList = filteredTasks.filter((task) => !checkedTaskIds.includes(task.id))
    ...
    <div className="fixed flex flex-row nowrap justify-between bottom-4 left-1/2 transform -translate-x-1/2 px-3 py-2 bg-[#1a1c20] border border-[#1a1c20] dark:border-[#fff] rounded-3xl gap-5">
      <div>
        <button className="flex flex-row justify-center items-center !text-[#fff] !text-[12px] md:!text-[16px] whitespace-nowrap gap-2 border border-[#fff] px-2 py-2 rounded-3xl">
          ...
          <span className="text-[15px] cursor-pointer" onClick={clearCheckedTasks}>
            <IoClose />
          </span>
        </button>
      </div>
      <div className="flex flex-row justify-center flex-nowrap gap-2">
        <button>
          <p className="!text-[#fff] !text-[12px] md:!text-[16px] whitespace-nowrap gap-2 border border-[#fff] px-4 py-2 rounded-3xl">
            Status
          </p>
        </button>
